<div class="container">
    ...
    <div class="stats-grid">
        <div class="stat-card">📝 Общее количество репортов: <strong>{{ $reports->count() }}</strong></div>
    </div>
    ...
    <table class="stats-table">
        ...
        <tbody>
            @foreach ($reports as $report)
                <tr>
                    <td>{{ $report->id }}</td>
                    <td>{{ $report->listing ? $report->listing->title : 'Не указано' }}</td>
                    <td>{{ $report->reason }}</td>
                    <td>{{ $report->details }}</td>
                    <td>{{ $report->created_at }}</td>
                    <td>
                        <!-- Кнопка "Перейти" для просмотра объявления -->
                        <a href="{{ route('listings.show', $report->listing_id) }}" class="btn btn-primary btn-sm direct"
                            target="_blank">Перейти</a>
                        <div class="manage">
                            <!-- Кнопка "Закрыть" для удаления репорта -->
                            <button class="btn btn-secondary btn-sm close"
                                onclick="closeReport({{ $report->id }})">Закрыть</button>

                            <!-- Кнопка "Удалить" для удаления объявления и репорта -->
                            <button class="btn btn-danger btn-sm delete"
                                onclick="deleteReport({{ $report->id }})">Удалить</button>
                        </div>
                    </td>
                </tr>
            @endforeach
